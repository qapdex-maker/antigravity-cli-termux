...
## 2026-08-08 - Robust Object-Type Verification to Prevent Fatal jq Indexing Crashes
**Vulnerability:** Standard `jq` safe navigation (e.g., `.parent?.child`) protects from indexing errors only if the parent field is missing or null. If the parent exists but is of a different, non-object type (such as a string, boolean, or number), standard indexing results in a fatal `jq` crash and halts wrapper execution.
**Learning:** When APIs or external agents return unexpected types in nested fields (e.g., due to field corruption or payload modification), direct indexing on those properties causes shell scripts using standard `jq` filters to exit immediately or lose critical visual cues.
**Prevention:** Always verify that parent objects are explicitly of type `"object"` before evaluation, using a robust helper like `safe_nested(parent_key; child_key): if (.[parent_key] | type) == "object" then .[parent_key][child_key] else null end` inside the custom `jq` filter definitions.

## 2026-08-11 - False-Positive Input Sanitization Fallback and State Masking via Transformed Non-ASCII Data
**Vulnerability:** Outputting pre-transformed non-ASCII data (such as emojis) directly from `jq` into shell state variables subjected to strict character allowlist validation (`[[ "$STATE" == *[!a-zA-Z0-9_-]* ]]`).
**Learning:** Because non-ASCII emojis fail standard ASCII character-set allowlist checks, the sanitization logic rejected valid states and reset `STATE` to a benign default (`idle`). This silently masked critical security and interactive states (such as `permission_required` or `input_required`) in window title outputs.
**Prevention:** Always emit raw, unformatted alphanumeric string values (e.g. `$st`) from JSON parsing filters into variables intended for character validation, deferring visual mappings to after validation or handling them safely within the allowlist rules.
